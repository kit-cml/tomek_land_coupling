...
void Tomek_model::___initConsts(double type)
{
...
void Tomek_model::initConsts(double type, double conc, double *hill)
{
	___initConsts(type);
	printf("Celltype: %lf\n", CONSTANTS[celltype]);
...
void Tomek_model::computeRates( double TIME, double *CONSTANTS, double *RATES, double *STATES, double *ALGEBRAIC, double land_trpn )
{
...
////IKr
  ...
  coeffs[8] = -  (CONSTANTS[beta_1]+ALGEBRAIC[alpha_2]+ALGEBRAIC[alpha_C2ToI]);
  coeffs[9] = CONSTANTS[alpha_1];
  coeffs[10] = CONSTANTS[beta_1];
  coeffs[11] = -  (ALGEBRAIC[beta]+CONSTANTS[alpha_1]);
